...
def create_path(root_dir, classifier_name, archive_name):
    output_directory = root_dir + os.sep+'results'+os.sep + classifier_name + os.sep + archive_name + os.sep
...
def read_dataset(root_dir, archive_name, dataset_name):
    ...
    if archive_name == 'mts_archive':
        file_name = cur_root_dir + os.sep+'archives'+os.sep + archive_name + os.sep + dataset_name + os.sep
        ...
        file_name = cur_root_dir + os.sep+'archives'+os.sep + archive_name + os.sep + dataset_name + os.sep + dataset_name
        x_train, y_train = readucr(file_name + '_TRAIN.txt')#xiugai
        x_test, y_test = readucr(file_name + '_TEST.txt')#xiugai
        datasets_dict[dataset_name] = (x_train.copy(), y_train.copy(), x_test.copy(),
                                       y_test.copy())
        print(datasets_dict[dataset_name])
# =============================================================================
#         #root_dir_dataset = cur_root_dir + os.sep+'archives'+os.sep + archive_name + os.sep + dataset_name + os.sep
#         
#         #df_train = pd.read_csv(root_dir_dataset + os.sep + dataset_name + '_TRAIN.tsv', sep='\t', header=None)
# 
#         #df_test = pd.read_csv(root_dir_dataset + os.sep + dataset_name + '_TEST.tsv', sep='\t', header=None)
#         root_dir_dataset = cur_root_dir + os.sep+'archives'+os.sep + archive_name + os.sep + dataset_name
#         df_train = pd.read_csv(root_dir_dataset + os.sep + dataset_name + '_TRAIN.ts', sep='\t', header=None)
# 
#         df_test = pd.read_csv(root_dir_dataset + os.sep + dataset_name + '_TEST.ts', sep='\t', header=None)
# 
...
def transform_mts_to_ucr_format():
    mts_root_dir = 'E:\文献\Self-Attention-Deep-Learing-test\ljw\\archives\mtsdata'
    mts_out_dir = 'E:\文献\Self-Attention-Deep-Learing-test\ljw\\archives\mts_archive'
    for dataset_name in MTS_DATASET_NAMES:
        # print('dataset_name',dataset_name)

        out_dir = mts_out_dir + os.sep+dataset_name + os.sep

        # if create_directory(out_dir) is None:
        #     print('Already_done')
        #     continue

        a = loadmat(mts_root_dir + os.sep+dataset_name +os.sep + dataset_name + '.mat')
        a = a['mts']
        a = a[0, 0]
...
def visualize_filter(root_dir):
    ...
    model = keras.models.load_model(
        root_dir + os.sep+'results'+os.sep+  classifier + os.sep + archive_name + os.sep + dataset_name + os.sep+'best_model.hdf5')
...
def viz_perf_themes(root_dir, df):
    ...
    for dataset_name in df.index:
        themes_index.append(constants.dataset_types[dataset_name])
...
def viz_cam(root_dir):
    ...
    for c in classes:
        ...
        plt.savefig(root_dir + os.sep+'temp'+os.sep + classifier + '-cam-' + save_name + '-class-' + str(int(c)) + '.png',
