...
class Classifier_RESNET_CONVATTENTION_CAUSAL:
    ...
    def build_model(self, input_shape, nb_classes):
        ...
        O_seq =tf.matmul(W,WV)
        
        
        W_Block=tf.reduce_sum(W, 1)
        W_Block = keras.layers.Activation('softmax')(W_Block)
        W_Block=tf.expand_dims(W_Block,axis=1)
        
# =============================================================================
#         W_Blocks=W_Block
#         for i in range(n_feature_maps-1):
#             W_Blocks=keras.layers.Concatenate(axis=1)([W_Blocks,W_Block])
# =============================================================================
            
        W_Blocks=[]
        for i in range(n_feature_maps):
            W_Blocks.append(W_Block)
        W_Blocks=keras.layers.Concatenate(axis=1)(W_Blocks)
        W_Blocks=tf.transpose(W_Blocks, perm=[0, 2, 1])
        W_time=W_Blocks*blocksrem
        O_seq=O_seq+W_time+blocksrem
        
        #O_seq=keras.layers.Concatenate(axis=2)([O_seq,W_time,blocksrem])
